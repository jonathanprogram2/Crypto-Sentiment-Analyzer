...
export default function TokenDetailClient({ symbol = "btc" }: { symbol?: string }) {
    ...
    useEffect(() => {
        const path = symbol.toLowerCase() === "btc" ? "/sample-btc.json" : "/sample-btc.json"; // fallback to BTC for PoC
        fetch(path)
            .then((r) => {
    ...
    if (error) return <p className="text-red-400 p-6" role="alert">Error: {error}</p>;
        ...
        <div className="max-w-6xl mx-auto px-6 py-10">
            <header className="flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    ...
                    {/* score badge */}
                    <div className="rounded-full border border-yellow-400/30 bg-yellow-400/10 px-4 py-1.5
                                    text-yellow-300 font-semibold shadow-[0_0_20px_-8px] shadow-yellow-400/60">
                        Score: {data.score}
                    </div>
            ...
            {modalOpen && (
                <div className="fixed inset-0 z-50">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center p-4">
                        <div className="w-full max-w-md rounded-2xl bg-slate-900 ring-1 ring-white/10 p-6">
                            <div className="flex items-center justify-between mb-3">
                                <h4 className="text-lg font-semibold">Why this score?</h4>
                                <button onClick={() => setModalOpen(false)} className="text-slate-400 hover:text-white">✕</button>
                            </div>
                            <ul className="list-disc pl-5 space-y-2 text-slate-300">
                                <li><span className="font-medium">Sources & weights:</span>Reddit, News, RSS (weights vary by quality)</li>
                                <li><span className="font-medium">Model:</span>FinBERT-like polarity; aggregated to a score</li>
                                <li><span className="font-medium">Quality filter:</span>dedupe, headline match, score span, credibility</li>
                            </ul>
                            <div className="mt-6 text-right">
                                <button onClick={() => setModalOpen(false)} className="rounded-lg bg-indigo-600 hover:bg-indigo-500 px-4 py-2">
                                    Close
                                </button>
                            </div>
                        </div>
